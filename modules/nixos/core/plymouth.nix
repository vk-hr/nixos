...
let
  ...
  script = pkgs.writeText "nixos.script" ''
    Window.SetBackgroundTopColor(${toString colors.base00-dec-r}, ${toString colors.base00-dec-g}, ${toString colors.base00-dec-b});
    Window.SetBackgroundBottomColor(${toString colors.base00-dec-r}, ${toString colors.base00-dec-g}, ${toString colors.base00-dec-b});
  ...
  theme = pkgs.stdenvNoCC.mkDerivation {
    ...
    buildPhase = ''
      ...
      magick -size 360x48 xc:none \
        -fill '#${colors.base01}' -stroke '#${colors.base03}' -strokewidth 1 \
        -draw 'roundrectangle 0,0 359,47 8,8' \
        entry.png

      magick -size 7x7 xc:none \
        -fill '#${colors.base05}' -draw 'circle 3.5,3.5 3.5,0' \
        bullet.png

      magick -size 64x64 xc:none \
        -stroke '#${colors.base05}' -strokewidth 5 -fill none \
        -draw 'arc 16,8 48,36 180,360' \
        -fill '#${colors.base05}' \
        -draw 'roundrectangle 14,30 50,56 5,5' \
        lock.png
    '';

    installPhase = ''
            mkdir -p $out/share/plymouth/themes/nixos
            cp ${script} $out/share/plymouth/themes/nixos/nixos.script
            cp logo.png progress_box.png progress_bar.png entry.png bullet.png lock.png \
              $out/share/plymouth/themes/nixos/

            cat > $out/share/plymouth/themes/nixos/nixos.plymouth <<EOF
      [Plymouth Theme]
      Name=NixOS
      Description=NixOS boot splash.
      ModuleName=script

      [script]
      ImageDir=$out/share/plymouth/themes/nixos
      ScriptFile=$out/share/plymouth/themes/nixos/nixos.script
      ConsoleLogBackgroundColor=0x${colors.base00}
      MonospaceFont=${config.stylix.fonts.sansSerif.name} 11
